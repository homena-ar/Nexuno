import { add_scroll_event, get_scroll_pos } from '../scroll/init';
    ...
    hide_bg_box: (y) => {
        ...
        gsap.to(icon, {
            ...
            onComplete: () => {
                ...
                if (icon.parentNode === undefined || icon.parentNode === null)
                    return;
        ...
        gsap.to(bg, {
            ...
            onComplete: () => {
                ...
                if (bg.parentNode === undefined || bg.parentNode === null)
                    return;
                // delete the background box
...
const position_nav_bg = (
    ...
    if (isRTL) {
        options.x =
            (document.documentElement.clientWidth - options.x - options.width) *
            -1;
    }
...
const mouseenter_handler = (e) => {
    ...
    if (bg === null || bg === undefined) {
        ...
        position_nav_bg(
            bg,
            coords,
            { x: el_coords.left - 25, y: el_coords.bottom },
            true,
            { autoAlpha: 1 }
        );
    }
    ...
    position_bg_icon(
        bg.querySelector('.icon'),
        (el_coords.right - el_coords.left + 15) / 2
    );
};
...
export const nav_handler = (e) => {
    ...
    if (e.type === 'mouseenter' && e.target.parentNode === nav)
...
export const add_nav_arrow = (selector) => {
    let items = document.querySelectorAll(selector);
    items.forEach((item) => {
        let anchor = item.querySelector(':scope > a');
        anchor.innerHTML +=
            '<div class="icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><title>arrow-right</title><path d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z" /></svg></div>';
    });
};
...
export const nav_slider_handler = () => {
    let menus = document.querySelectorAll(
        '.navigation-menu-wrapper.desktop-wrapper.swiper-container'
    );
    menus.forEach((menu) => {
        let id = Math.floor(Math.random() * (99999 - 0 + 1)) + 0;

        let prev = document.createElement('div');
        prev.className = `menu-slider-nav menu-slider-nav-prev menu-slider-nav-prev-${id}`;
        prev.innerHTML =
        ...
        menu.parentNode.prepend(prev);

        let next = document.createElement('div');
        next.className = `menu-slider-nav menu-slider-nav-next menu-slider-nav-next-${id}`;
        next.innerHTML =
        ...
        new Swiper(menu, {
            ...
            speed: 500,
            navigation: {
                prevEl: `.menu-slider-nav-prev-${id}`,
                nextEl: `.menu-slider-nav-next-${id}`,
            },
        });

        Array.from(menu.swiper.slides).forEach((slide, index) => {
            if (
                slide.classList.contains('current-menu-item') ||
                slide.classList.contains('current-menu-ancestor')
            ) {
                menu.swiper.slideTo(index, 1000);
            }
        });
    });
};
